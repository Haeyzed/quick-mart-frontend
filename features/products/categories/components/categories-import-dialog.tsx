...
export function CategoriesImportDialog({
  ...
  const onSubmit = async (data: z.infer<typeof importSchema>) => {
    try {
      const response = await importCategories.mutateAsync(data.file)
    ...
    >
      <DialogContent className='sm:max-w-md' onPointerDownOutside={(e) => e.preventDefault()} onEscapeKeyDown={(e) => e.preventDefault()}>
        <DialogHeader className='text-start'>
          ...
          <DialogDescription>
            Upload a CSV or Excel file to import categories.
          </DialogDescription>
        </DialogHeader>
        <form
        ...
        <DialogFooter className='gap-y-2'>
          ...
          <Button type='submit' form='category-import-form' disabled={importCategories.isPending}>
            {importCategories.isPending ? 'Importing...' : 'Import'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
