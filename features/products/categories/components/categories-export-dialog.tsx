...
import { useExportCategories } from '../api/use-categories'
...
const AVAILABLE_COLUMNS = [
  ...
  { value: 'parent_name', label: 'Parent Category' },
  { value: 'featured', label: 'Featured' },
  { value: 'is_active', label: 'Is Active' },
  { value: 'is_sync_disable', label: 'Is Sync Disabled' },
  { value: 'created_at', label: 'Created At' },
  { value: 'updated_at', label: 'Updated At' },
] as const

type CategoriesExportDialogProps = {
  open: boolean
...
export function CategoriesExportDialog({
  ...
  const form = useForm<z.infer<typeof exportSchema>>({
    ...
    defaultValues: {
      format: 'excel',
      method: 'download',
      columns: ['id', 'name', 'slug', 'parent_name', 'is_active'],
    },
  })
      ...
      <DialogContent className='sm:max-w-2xl max-h-[90vh] overflow-y-auto' onPointerDownOutside={(e) => e.preventDefault()} onEscapeKeyDown={(e) => e.preventDefault()}>
        <DialogHeader className='text-start'>
          ...
          <DialogDescription>
            ...
            {ids.length > 0 && ` ${ids.length} categor${ids.length > 1 ? 'ies' : 'y'} selected.`}
          </DialogDescription>
        </DialogHeader>
        <form
        ...
        <DialogFooter className='gap-y-2'>
          ...
          <Button
            ...
            disabled={exportCategories.isPending || isLoadingUsers}
          >
            {exportCategories.isPending ? (
              <>
