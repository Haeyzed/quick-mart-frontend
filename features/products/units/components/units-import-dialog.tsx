...
export function UnitsImportDialog({
  ...
  const onSubmit = async (data: z.infer<typeof importSchema>) => {
    try {
      const response = await importUnits.mutateAsync(data.file)
    ...
    >
      <DialogContent className='sm:max-w-md' onPointerDownOutside={(e) => e.preventDefault()} onEscapeKeyDown={(e) => e.preventDefault()}>
        <DialogHeader className='text-start'>
          ...
          <DialogDescription>
            Upload a CSV or Excel file to import units.
          </DialogDescription>
        </DialogHeader>
        <form
          id='unit-import-form'
        ...
        <DialogFooter className='gap-y-2'>
          ...
          <Button type='submit' form='unit-import-form' disabled={importUnits.isPending}>
            {importUnits.isPending ? 'Importing...' : 'Import'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
