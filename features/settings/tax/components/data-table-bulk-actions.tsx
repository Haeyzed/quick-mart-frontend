...
export function DataTableBulkActions<TData>({
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      const message = (response as any)?.message || `Deleted ${ids.length} tax${ids.length > 1 ? 'es' : ''}`
  ...
  const handleBulkActivate = async () => {
    try {
      ...
      table.resetRowSelection()
      const message = (response as any)?.message || `Activated ${ids.length} tax${ids.length > 1 ? 'es' : ''}`
      toast.success(message)
    } catch (error: any) {
      handleApiError(error)
    }
